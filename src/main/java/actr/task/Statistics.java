...
/**
 * The statistics module that includes helper functions for statistical analysis of task performance.
 *  
...
public class Statistics
{
	 ...
	 */
	public static double stderr (double a[]) { return Math.sqrt (stddev(a)); }
	 ...
	 */
	public static double confidence (double a[]) { return 1.96 * stderr(a); }
	...
	{
		if (a1.length<=1 || (a1.length != a2.length)) return 0;
		double ma = mean(a1), mb = mean(a2);
		double sda = stddev(a1), sdb = stddev(a2);
		double sum = 0;
		for (int i=0 ; i<a1.length ; i++) sum += (a1[i]-ma)*(a2[i]-mb);
		return sum / ((a1.length-1) * sda * sdb);
	}

	/**
	 * Computes the correlation of two two-dimensional arrays (by flattening them and then computing
	 * the correlation on the flattened arrays).
	 * @param a1 the first array
	 * @param a2 the second array
	 * @return the correlation
	 */
	...
	public static double rmse (double a1[], double a2[])
	{
		if (a1.length<=1 || (a1.length != a2.length)) return 0;
		double sum = 0;
		for (int i=0 ; i<a1.length ; i++) sum += (a1[i]-a2[i]) * (a1[i]-a2[i]);
		return Math.sqrt (sum / a1.length);
	}

	/**
	 * Computes the RMSE of two two-dimensional arrays (by flattening them and then computing
	 * the RMSE on the flattened arrays).
	 * @param a1 the first array
	 * @param a2 the second array
	 * @return the RMSE
	 */
	...
	/**
	 * Computes a normalized error of two arrays as the proportion of the RMSE to the mean.
	 * @param model the model data
	 * @param human the human data
	...
	/**
	 * Computes a normalized error of two 2d arrays as the proportion of the RMSE to the mean.
	 * @param model the model data
	 * @param human the human data
	 * @return the normalized error
	 */
