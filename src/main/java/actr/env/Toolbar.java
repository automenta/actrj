...
class Toolbar extends JToolBar
{
	...
	JButton addButton (Action action) { return addButton (action, 26); }

	JButton addDropButton (String title, final JButton primary, String[][] options,
			int defaultOption, final Setter setter)
	{
		...
		for (int i=0 ; i<options.length ; i++)
		{
			...
			item.addActionListener (new ActionListener() {
				public void actionPerformed (ActionEvent e) { setter.set (pair[1]); }
			});
			group.add (item);
			menu.add (item);
			if (i == defaultOption)
	...
	String getSpeedup () { return speedup; }
	String getIterations () { return iterations; }
}
