...
class Preferences
{
	...
	private static final String prefsFilePath =
		((Main.inApplet()) ? null
	...
	{
		if (Main.inApplet()) return new Preferences();
		...
		try
		{
			...
			for (int i=0 ; i<lines.length ; i++)
			{
				...
				if (var.equals("frameWidth")) prefs.frameWidth = Integer.valueOf(val);
				else if (var.equals("frameHeight")) prefs.frameHeight = Integer.valueOf(val);
				else if (var.equals("editorPaneSplit")) prefs.editorPaneSplit = Integer.valueOf(val);
				else if (var.equals("taskPaneSplit")) prefs.taskPaneSplit = Integer.valueOf(val);
				else if (var.equals("font")) prefs.font = val;
				else if (var.equals("fontSize")) prefs.fontSize = Integer.valueOf(val);
				else if (var.equals("autoHilite")) prefs.autoHilite = val.equals("true");
				else if (var.equals("autoTab")) prefs.autoIndent = val.equals("true");
				else if (var.equals("commandColor")) prefs.commandColor = Color.decode(val);
				else if (var.equals("parameterColor")) prefs.parameterColor = Color.decode(val);
				else if (var.equals("productionColor")) prefs.productionColor = Color.decode(val);
				else if (var.equals("chunkColor")) prefs.chunkColor = Color.decode(val);
				else if (var.equals("bufferColor")) prefs.bufferColor = Color.decode(val);
				else if (var.equals("commentColor")) prefs.commentColor = Color.decode(val);
				else if (var.equals("tabSpaces")) prefs.indentSpaces = Integer.valueOf(val);
	...
	void save ()
	{
		...
		try
		{
			...
			if (recentFiles.size() > 0) pw.print (recentFiles.elementAt(0));
	...
	{
		if (recentFiles.size() > 0) return recentFiles.elementAt(0);
		else return null;
	}
	...
	void addRecentFile (String fileName)
	{
		...
		if (recentFiles.size() > maxRecentFiles) recentFiles.removeElementAt (recentFiles.size()-1);
	}
}
