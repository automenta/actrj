...
public class Declarative extends Module
{
	...
	Declarative (Model model)
	{
		...
		{
			try { extendedMemory = extendedMemoryClass.newInstance(); }
	...
	Chunk findRetrieval (Chunk request)
	{
		...
		if (matches.isEmpty())
		{
			...
			if (extendedMemory!=null)
			{
				...
				if (model.isVerbose())
				{
					if (chunk!=null) model.output ("declarative", "extended memory: found " + chunk);
					else model.output ("declarative", "extended memory: failure");
				}
			}
			if (chunk==null && activationTrace) model.output ("*** no matching chunks");
			return chunk;
		}
		else
		{
	...
	/** 
	 * Sets the class to be used for new instances of an extended declarative memory.
	 * @param emClass the name of the extended memory class
	 */
