...
public class Motor extends Module
{
	...
	private void incrementalMove (final IncrementalMove im)
	{
		...
		model.addEvent (new Event (eventTime, "actr/task", "update") {
			public void action() {
	...
	void update ()
	{
		...
		if (request.get(Symbol.isa) == Symbol.get("type"))
		{
			...
			if (stringsym==null) { model.outputWarning ("string not specified"); return; }
			String s = stringsym.getString().replaceAll ("\"", "").toLowerCase();
			if (!seen && model.verboseTrace) model.output ("motor", "type "+s);
			queuedMovements.removeAllElements();
			for (int i=0 ; i<s.length() ; i++)
			{
				String key = "" + s.charAt(i);
				String slots = getKeyCommand (key);
				if (slots==null) { model.outputWarning ("bad key '"+key+"'"); return; }
		...
		if (request.get(Symbol.isa) == Symbol.get("press-key"))
		{
			...
			if (keysym==null) { model.outputWarning ("key not specified"); return; }
		...
		if (request.get(Symbol.isa) == Symbol.get("move-cursor"))
		{
			...
			if (!rightHand.equals(mouseKey)) { model.outputWarning ("hand not on mouse"); return; }
			...
			if (model.realTime)
				model.addEvent (new Event (eventTime, "actr/task", "none") {
					public void action() {
		...
		else if (request.get(Symbol.isa) == Symbol.get("point-hand-at-key"))
		{
			...
			if (toKeySym==Symbol.nil) { model.outputWarning ("to-key not specified"); return; }
		...
		else if (request.get(Symbol.isa) == Symbol.get("punch"))
		{
			...
			if (hand==Symbol.right && rightHand.equals(mouseKey)) { model.outputWarning ("hand not on keyboard"); return; }
			...
			model.addEvent (new Event (eventTime, "motor", "output key "+key) {
				public void action() {
					if (key.equals("mouse")) model.getTask().clickMouse();
					else model.getTask().typeKey (convertToChar (key));
				}
			});
			eventTime -= keyClosureTime;
			eventTime += 2*burstTime;
	...
	private char convertToChar (String keyname)
	{
		...
		else if (keyname.equals("return")) return '\n';
		else if (keyname.equals("period")) return '.';
		else if (keyname.equals("comma")) return ',';
