...
public class Vision extends Module
{
	...
	private class VisualObject
	{
		...
		{
			return "(" + id + " kind " + kind + " screen-x " + x + " screen-y " + y
					+ " value " + value + " :attended " + (attended ? "t" : "nil") + ")";
		}
	}
	...
	Chunk getVisualLocation (Symbol name)
	{
		...
		else return model.getDeclarative().get(name);
	}
	...
	/**
	 * Adds a visual object for access by the vision module. The identifier should be unique for different
	 * sounds; if the same identifier is used, the new object will cancel the previous one
	 * (as might be useful for the movements of a single object).
	 ...
	 * @param x the x coordinate of the visual object (i.e., the "screen-x" slot value)
	 * @param y the y coordinate of the visual object (i.e., the "screen-y" slot value)
	...
	public void addVisual (String id, String type, String value, int x, int y, int w, int h, double d)
	{
		...
		if (model.bufferStuffing
				&& (model.getBuffers().get(Symbol.visloc)==null
	 ...
	 */
	public int getEyeX () { return eyeX; }

	/**
	 * Gets the y coordinate of the eye's current location.
	 */
	public int getEyeY () { return eyeY; }
	...
	private boolean matchesVisualObject (Chunk request, VisualObject vo)
	{
		...
		while (it.hasNext())
		{
			...
			if (slot==Symbol.kind)
			{ if (value!=vo.kind) return false; else continue; }
			...
			if (value == Symbol.current)
			{
				...
				{
					if (slot.getString().contains("-x")) vi = lastEncodedVisObj.x;
					else if (slot.getString().contains("-y")) vi = lastEncodedVisObj.y;
				}
			}
			else
			{
	...
	private double computeEncodingTime (VisualObject vo)
	{
		...
		if (noise < -2.0*t_enc/3.0) noise = -2.0*t_enc/3.0;
		t_enc += noise;
		return t_enc;
	}
	...
	void update ()
	{
		...
		if (request!=null && request.isRequest())
		{
			...
			if (visloc != null)
			{
				if (model.verboseTrace) model.output ("vision", "find-location [" + visloc.getName().getString() + "]");
				model.getBuffers().set (Symbol.visloc, visloc);
				model.getBuffers().setSlot (Symbol.vislocState, Symbol.state, Symbol.free);
		...
		if (request!=null && request.isRequest() && request.get(Symbol.isa)==Symbol.get("move-attention"))
		{
			...
			if (vislocName == null) { model.outputWarning ("bad visual location"); return; }
			final VisualObject vo = vislocs.get (vislocName);
			Chunk visloc = vo.visloc;
			if (visloc == null) { model.outputWarning ("bad visual location"); return; }
			...
			model.addEvent (new Event (model.getTime() + encodingTime,
					"vision", "encoding-complete ["+visual.getName()+"]")
			{
				public void action() {
			...
			if (useEMMA) prepareEyeMovement (vo, visual);
		}
	}
	...
	private void prepareEyeMovement (final VisualObject vo, final Chunk visual)
	{
		...
		model.addEvent (new Event (model.getTime() + emmaPreparationTime,
				"eye", "preparation-complete ["+visual.getName()+"]")
		{
			public void action() {
	...
	private void executeEyeMovement (final VisualObject vo, final Chunk visual)
	{
		...
		model.addEvent (new Event (model.getTime() + executionTime,
				"eye", "execution-complete ["+visual.getName()+"]")
		{
			public void action() {
				...
				if (vo.encodingStart + vo.encodingTime > model.getTime())
				{
					...
					if (remainingEncodingTime <= 0) return;
					model.changeEventTime ("vision", "encoding-complete", model.getTime() + remainingEncodingTime);
					vo.encodingStart = model.getTime();
