...
public class Bold
{
	...
	private final Symbol buffers[] = {Symbol.retrieval, Symbol.imaginal, Symbol.visual, Symbol.aural, Symbol.manual, Symbol.vocal};
	...
	private class Span
	{
		double startTime;
		boolean active;
		Span (double st, boolean a) { startTime=st; active=a; }
	}
	...
	private class Activity
	{
		...
		{ 
			...
			{
				if (last.startTime != time) spans.add (new Span (time, active));
				else last.active = active;
			}
		}
	...
	public boolean isImaging () { return true; }
	...
	void recordActivity (Symbol buffer)
	{
		...
		Activity activity = activities.get (buffer);
		if (activity != null) activity.add (model.getTime(), active);
	}
	...
	{
		if (end < current-boldSettle) return 0;
	...
	{
		double factorial = 1;
		for (int i=2 ; i<=boldExponent ; i++) factorial *= i;
	...
	{
		Activity activity = activities.get (buffer);
		if (activity != null) return cumulative (activity);
		else return 0;
	}
}
