...
/**
 * Chunks of declarative knowledge represented as pairings of slots and slot values.
 * <p>
 * Unlike the classic ACT-R architecture, this implementation does <i>not</i> use chunk types to
 * specify the type of each chunk via a special <tt>isa</tt> slot. Instead, the model can assign
 * a slot value to any slot name (including using <tt>isa</tt> in the traditional way, thus
 * ensuring compatibility with older models).
 *  
...
public class Chunk
{
	...
	boolean isRequest () { return isRequest; }
	boolean isRetrieved () { return retrieved; }
	double getRetrievalTime () { return retrievalTime; }
	double getLastUsedAsGoal () { return lastUsedAsGoal; }
	
	void setName (Symbol name) { this.name = name; }
	void setRequest (boolean b) { isRequest = b; }
	void setRetrieved (boolean b) { retrieved = b; }
	void setRetrievalTime (double time) { retrievalTime = time; }
	void setLastUsedAsGoal (double time) { lastUsedAsGoal = time; }

	/**
	 * Gets the value of the given slot, or <tt>Symbol.nil</tt> if the slot is undefined.
	 ...
	 */
	public Iterator<Symbol> getSlotNames() { return slots.keySet().iterator(); }
	
	/**
	 * Gets an iterator to iterate through all slot values.
	 * @return an iterator for slot values
	 */
	public Iterator<Symbol> getSlotValues() { return slots.values().iterator(); }
	...
	public void set (Symbol slot, Symbol value)
	{
		...
		if (value==Symbol.nil && !slot.getString().startsWith(":")) slots.remove (slot);
		else
		{
	...
	/**
	 * Checks if another chunk is equal to this one by checking the equality of all slots and values.
	 * @param c2 the chunk for comparison
	 * @return <tt>true</tt> if the chunks are equal
	 */
	public boolean equals (Chunk c2)
	{
		if (slotCount()==0 && c2.slotCount()==0) return (name == c2.getName());
		if (slotCount() != c2.slotCount()) return false;
	...
	{
		if (!model.getDeclarative().baseLevelLearning) return baseLevel;
	...
	{
		if (cj.appearsInSlotsOf(ci)==0 && cj.getName()!=ci.getName()) return 0;
		else return model.getDeclarative().maximumAssociativeStrength - Math.log(cj.fan);
	}
	...
	double computeSpreadingActivation (Chunk goal, double totalW)
	{
		...
		{
			Symbol slot = it.next();
			if (slot==Symbol.isa || slot.getString().startsWith(":")) continue;
	...
	double computeActivation (Chunk request)
	{
		...
		if (model.getDeclarative().spreadingActivation)
		{
			if (model.getDeclarative().goalActivation > 0)
			{
				...
				if (goal != null) activation += computeSpreadingActivation (goal, model.getDeclarative().goalActivation);
			}
			if (model.getDeclarative().imaginalActivation > 0)
			{
				Chunk imaginal = model.getBuffers().get (Symbol.imaginal);
				if (imaginal != null) activation += computeSpreadingActivation (imaginal, model.getDeclarative().imaginalActivation);
			}
		}
	...
	int getUseCount ()
	{
		if (model.getDeclarative().optimizedLearning) return useCount;
	...
	void assignNewThreadID () { threadID = model.getNewThreadID(); }
	...
	/** 
	 * Gets a string representation of the chunk showing its slot names and values. 
	 * @return the string
	 */
