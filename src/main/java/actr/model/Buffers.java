...
public class Buffers
{
	...
	/**
	 * Checks whether the given symbol is a legal buffer symbol by checking for the presence
	 * of a state buffer for that symbol.
	...
	/**
	 * Checks whether the specified buffer is a state buffer with a <tt>'?'</tt> prefix.
	 ...
	 * @param buffer the buffer name (without a prefix for normal buffers but with the '?' prefix for state buffers)
	 ...
	 * @return the value of the slot, or <tt>null</tt> if the buffer or slot is empty
	 */
	...
	void set (Symbol buffer, Chunk c)
	{
		if (buffer == Symbol.goal)
		{
			if (lastGoalSetTime < model.getTime())
			{
				...
				if (oldGoal != null) c.setThreadID (oldGoal.getThreadID());
				else c.assignNewThreadID();
		...
		if (model.getBold().isImaging()) model.getBold().recordActivity (buffer);
	}
	...
	void setSlot (Symbol buffer, Symbol slot, Symbol value)
	{
		...
		if (!isState(buffer)) touch (buffer);
	...
	void clear (Symbol buffer)
	{
		...
		else if (buffer == Symbol.manual) model.removeEvents ("motor");
		else if (buffer == Symbol.vocal) model.removeEvents ("speech");
	...
	int numGoals () { return goals.size(); }
	...
	/** 
	 * Gets a string representation of the buffers showing existing buffers and their content. 
	 * @return the string
	 */
