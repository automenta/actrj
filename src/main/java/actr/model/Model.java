...
public class Model
{
	...
	public void run (boolean reset)
	{
		...
		while (!stop && (events.hasMoreEvents() || runUntilStop))
		{
			...
			taskUpdated = false;
			if (verboseTrace && !event.getModule().equals("actr/task")
			...
			if (!event.getModule().equals("procedural")
					...
					&& (taskUpdated || !event.getModule().equals("actr/task"))
	...
	void setParameter (String parameter, String value, Tokenizer t)
	{
		...
		else if (parameter.equals(":brain-imaging")) { } //bold.isImaging() = (!value.equals("nil"));
